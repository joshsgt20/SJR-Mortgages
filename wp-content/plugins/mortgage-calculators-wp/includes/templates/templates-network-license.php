<?php
	...
	<div class="wrap">
        ...
        if (! isset($_GET['action']) || isset($_GET['action']) && 'cal-one' != $_GET['action']  && 'cal-two' != $_GET['action']  && 'cal-three' != $_GET['action']  && 'cal-four' != $_GET['action']) {
            ...
            <?php /*
            <a href="<?php echo esc_url(add_query_arg(array( 'action' => 'cal-four' ), admin_url($admin_url))); ?>" class="nav-tab<?php if (cal_four_Screen) {
            echo ' nav-tab-active';
            } ?>"><?php esc_html_e('Calculator 4'); ?></a>
            */ ?>
		...
		<div class="wrap">
			...
			<form method="post" action="<?php echo(is_network_admin() ? 'edit.php?action=wpmc_update_network_options' : 'options.php')?>"><?php
                 if ($cal_one_screen) {
    ...
    function wpmc_calculator_template1()
    {
        ...
        //$wpmc_one_mt_initial = $option_func('wpmc_one_mt_initial');
